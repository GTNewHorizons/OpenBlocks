...
public class TileEntityGuide extends DroppableTileEntity implements ISyncListener, INeighbourAwareTile, IAddAwareTile {
    ...
    private void displayModeChange(EntityPlayer player) {
        player.addChatMessage(
                new ChatComponentTranslation("openblocks.misc.change_mode", getCurrentMode().getLocalizedName()));
    }
    ...
    public void onSync(Set<ISyncableObject> changes) {
        if (changes.contains(negX) || changes.contains(negY)
                || changes.contains(negZ)
                || changes.contains(posX)
                || changes.contains(posY)
                || changes.contains(posZ)
