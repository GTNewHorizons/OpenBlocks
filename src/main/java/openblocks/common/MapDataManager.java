...
public class MapDataManager {
    ...
    public abstract static class MapIdRequest extends NetworkEvent {
        ...
        protected void writeToStream(DataOutput output) {
            ...
            for (Integer id : mapIds) ByteUtils.writeVLI(output, id);
        }
    }
    ...
    public void onMapDataRequest(MapDataRequestEvent evt) {
        ...
        if (!missingMaps.isEmpty()) {
            ...
            if (Config.alwaysReportInvalidMapRequests || !lessThan16 || !Loader.isModLoaded(Mods.NOTENOUGHITEMS))
                Log.info("Player %s asked for non-existent maps %s", evt.sender, missingMaps.toString());
        }
