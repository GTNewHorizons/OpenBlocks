...
public class PlayerInventoryStore {
    ...
    private static ExtrasFiller createExtrasFiller(final GameProfile profile, final double x, final double y,
            final double z, final Map<String, SubInventory> subs) {
    ...
    public void onPlayerDeath(LivingDeathEvent event) {
        if (Config.dumpStiffsStuff && (event.entity instanceof EntityPlayerMP)
            ...
            try {
                ...
                Log.info(
                        "Storing post-mortem inventory into %s. It can be restored with command '/ob_inventory restore %s %s'",
                        file.getAbsolutePath(),
                        playerName,
                        stripFilename(file.getName()));
            } catch (Exception e) {
