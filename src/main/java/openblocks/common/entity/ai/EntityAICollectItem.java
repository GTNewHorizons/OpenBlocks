        ...
        if (luggage.worldObj != null) {
            ...
            List<EntityItem> items = luggage.worldObj.getEntitiesWithinAABB(
                    ...
                    AxisAlignedBB.getBoundingBox(
                            ...
                            luggage.posX + 1,
                            luggage.posY + 1,
                            luggage.posZ + 1).expand(10.0, 10.0, 10.0));
            EntityItem closest = null;
            double closestDistance = Double.MAX_VALUE;
            for (EntityItem item : items) {
                if (!item.isDead && item.onGround) {
                    double dist = item.getDistanceToEntity(luggage);
                    if (dist < closestDistance && luggage.canConsumeStackPartially(item.getEntityItem())
                            && !item.isInWater()) {
                        closest = item;
            ...
            if (targetItem != null && luggage.getDistanceToEntity(targetItem) < 1.0) {
                ...
                if (preEatSize != stack.stackSize) {
                    if (luggage.lastSound > 15) {
                        ...
                        luggage.playSound(
                                isFood ? "openblocks:luggage.eat.food" : "openblocks:luggage.eat.item",
                                0.5f,
