...
public class PlayerDeathHandler {
    ...
    private static class SearchOrder implements Iterable<Coord> {
        ...
        public SearchOrder(int size) {
            ...
            for (int x = -size; x <= size; x++)
                for (int y = -size; y <= size; y++) for (int z = -size; z <= size; z++) coords.add(new Coord(x, y, z));
    ...
    private static class GraveCallable implements Runnable {
        ...
        private boolean tryPlaceGrave(World world, final int x, final int y, final int z, String gravestoneText,
            ...
            if (tile == null || !(tile instanceof TileEntityGrave)) {
                Log.warn(
                        "Failed to place grave @ %d,%d,%d: invalid tile entity: %s(%s)",
                        x,
                        y,
                        z,
                        tile,
                        tile != null ? tile.getClass() : "?");
                return false;
            }
        ...
        private boolean trySpawnGrave(EntityPlayer player, World world) {
            ...
            final GraveSpawnEvent evt = location == null ? new GraveSpawnEvent(player, loot, gravestoneText, cause)
        ...
        private static boolean canSpawnBase(World world, EntityPlayer player, int x, int y, int z) {
            return world.blockExists(x, y, z) && world.getBlock(x, y, z).isAir(world, x, y, z)
        ...
        private void backupGrave(World world, IInventory loot, ExtrasFiller filler) {
            try {
                File backup = PlayerInventoryStore.instance
                        .storeInventory(loot, stiffId.getName(), "grave", world, filler);
        ...
        public void run() {
            ...
            if (!trySpawnGrave(player, world)) {
                ...
                for (EntityItem drop : loot) world.spawnEntityInWorld(drop);
            }
        }
    }
    ...
    public void onPlayerDrops(PlayerDropsEvent event) {
        ...
        for (EntityItem drop : drops) dropsEvent.addItem(drop);
        ...
        if (graveLoot.isEmpty()) {
            Log.log(
                    debugLevel(),
                    "No grave drops left for player '%s' after event filtering, grave will not be spawned'",
                    player);
            return;
        }
        ...
        Log.log(
                debugLevel(),
                "Scheduling grave placement for player '%s':'%s' with %d item(s) stored and %d item(s) dropped",
                player,
    ...
    private static void dumpDebugInfo(PlayerDropsEvent event) {
        ...
        int i = 0;
        for (EntityItem e : event.drops) Log.info("\tGrave drop %d: %s -> %s", i++, e.getClass(), e.getEntityItem());
