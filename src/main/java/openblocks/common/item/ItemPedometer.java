...
public class ItemPedometer extends Item {
    ...
    protected void showPedometerData(EntityPlayer player, PedometerState state) {
        ...
        send(
                player,
                "openblocks.misc.pedometer.start_point",
                String.format(
                        "%.1f %.1f %.1f",
                        result.startingPoint.xCoord,
                        result.startingPoint.yCoord,
                        result.startingPoint.zCoord));
        ...
        send(
                player,
                "openblocks.misc.pedometer.straght_line_distance",
                distanceUnit.format(result.straightLineDistance));
    ...
    public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
        if (player.motionX * player.motionX + player.motionY * player.motionY + player.motionZ * player.motionZ > 0.01)
            return pedometerMoving;
