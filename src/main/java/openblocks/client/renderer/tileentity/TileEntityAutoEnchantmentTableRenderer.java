...
public class TileEntityAutoEnchantmentTableRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float f) {
        ...
        while (f2 >= (float) Math.PI) f2 -= 2 * (float) Math.PI;
        ...
        float f4 = table.pageFlipPrev + (table.pageFlip - table.pageFlipPrev) * f + 0.25F;
        float f5 = table.pageFlipPrev + (table.pageFlip - table.pageFlipPrev) * f + 0.75F;
